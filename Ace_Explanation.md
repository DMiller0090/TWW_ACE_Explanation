...
4. Switch back to camera mode, but press **B** while the transition is occuring, this gives Link a few frames to move around and interact with the NPC, and while the pictobox is open, no textboxes from NPCs can be created.  
   a.) Talk to NPC while the camera is transitioning to camera mode, switch back to gallery mode immediately to keep the pictobox open  
   b.) Switch back to camera mode, press **B** again to be able to walk away from the NPC.  
   c.) Repeat switching back and forth until the actor is offscreen.  
...
2. Once the final text box of the first NPCs has completed, this is what happens:  
   a.) The NPC sees the final text prompt has been closed and requests to delete the textbox by doing the following:  
...
7. The pictobox UI (user interface) data loads into the now freed space in memory that just got cleared earlier from the 1st NPC.  
   a.) ```sScreen``` is still pointing to the same place in memory, but instead of pointing to screen data, it now points to (seemingly random) from the pictobox.  
...
Relating back to our example, we mentioned that ```sScreen``` is refering to the address 0x0x81572C40. Originally, 0x0x81572C40 stored information about the (now deleted) textboxes screen data. However, now it's being populated by pictobox data. But what pictobox data exactly is getting loading into that address?  

It just so happens to be a certain collection of pixels within the 3rd image saved in the pictobox.  

> **Image Placeholder** (see image attached)
...
1. The pixel data just so happens to form an address, 0x80ABBDFF (seem image)  
